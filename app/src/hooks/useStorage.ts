...
function makeUseStorage(storage: Storage, cache: Record<string, any>, name: string = storage.constructor.name) {
	return function useStorage<T>(key: string, initialValue: T, writeInitialValue = false) {
...
function throttle(func: (...args: any[]) => any, wait: number, options?: { trailing?: boolean; leading?: boolean }): (...args: any[]) => any {
	let previous = 0;
