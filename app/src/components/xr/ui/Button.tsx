...
export const Button: (props: ButtonProperties & RefAttributes<ContainerRef>) => ReactNode = forwardRef(
	({ children, variant = 'default', size = 'default', disabled = false, hover, animated = true, ...props }, ref) => {
		...
		const { spring } = useSpring({ spring: 0, config: config.default, ref: api });
		...
		return (
			<AnimatedContainer
				...
				ref={ref}
			>
				<PositionalAudio url={`./audio/click.webm`} distance={audioDistance[size]} autoplay={false} loop={false} ref={audioRef} />
				<DefaultProperties fontSize={16} lineHeight={24} fontWeight="medium" wordBreak="keep-all" {...defaultProps} opacity={disabled ? 0.5 : undefined}>
					{children}
				</DefaultProperties>
			</AnimatedContainer>
		);
