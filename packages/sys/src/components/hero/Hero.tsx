...
export const HeroRoot = forwardRef<HTMLDivElement, HeroProps>(function HeroRoot({ className, big, padded, ...props }, ref) {
	const innerRef = useHeightGlobal('--hero-height');
	const finalRef = useMergedRef(ref, innerRef);
	return <Box ref={finalRef} {...props} className={clsx(cls.root, big && cls.big, padded === false && cls.noPadding, className)} />;
});
